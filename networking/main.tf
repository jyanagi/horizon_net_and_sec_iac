...
resource "nsxt_policy_vlan_segment" "nsx-vlan-fa-seg" {
  ...
  description         = "Fabric-A VLAN Segment created by Terraform"
  transport_zone_path = data.nsxt_policy_transport_zone.vlan_tz.path
  vlan_ids            = [var.segment_fa_vlan_id]

  advanced_config {
    uplink_teaming_policy = var.teaming_policy_a
  }
...
resource "nsxt_policy_vlan_segment" "nsx-vlan-fb-seg" {
  ...
  description         = "Fabric-B VLAN Segment created by Terraform"
  transport_zone_path = data.nsxt_policy_transport_zone.vlan_tz.path
  vlan_ids            = [var.segment_fb_vlan_id]

  advanced_config {
    uplink_teaming_policy = var.teaming_policy_b
  }
...
resource "nsxt_policy_gateway_redistribution_config" "t0-route-redistribution" {
